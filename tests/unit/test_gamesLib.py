from games.gamesLib.services import filter_games, filter_games_by_genre
...
def test_filtering_games_by_genre_match():
    batch_of_games = [{'title': 'Game 1', 'genres': [Genre('Casual'), Genre('Free to Play'), Genre('Indie')]},
                      {'title': 'Game 2', 'genres': [Genre('Adventure'), Genre('Action'), Genre('Indie')]}]
    target_genre = 'Indie'

    result = filter_games_by_genre(batch_of_games, target_genre)

    assert result == [{'title': 'Game 1', 'genres': [Genre('Casual'), Genre('Free to Play'), Genre('Indie')]},
                      {'title': 'Game 2', 'genres': [Genre('Adventure'), Genre('Action'), Genre('Indie')]}]


def test_filtering_games_by_genre_no_match():
    batch_of_games = [{'title': 'Game 1', 'genres': [Genre('Casual'), Genre('Free to Play'), Genre('Indie')]},
                      {'title': 'Game 2', 'genres': [Genre('Adventure'), Genre('Action'), Genre('Indie')]}]
    target_genre = Genre('RPG')
...
def test_filtering_games_by_genre_one_matched():
    batch_of_games = [{'title': 'Game 1', 'genres': [Genre('Casual'), Genre('Free to Play'), Genre('Indie')]},
                       {'title': 'Game 2', 'genres': [Genre('Adventure'), Genre('Action'), Genre('Indie')]}]
    target_genre = 'Casual'
